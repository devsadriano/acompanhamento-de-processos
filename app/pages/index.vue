...
const getStatusBadgeClass = (status: string) => {
  switch (status) {
    ...
    case 'COBRAR': return 'bg-orange-50 text-orange-700 border-orange-200'
    case 'PENDENTE': return 'bg-gray-50 text-gray-700 border-gray-200'
    case 'AGUARDANDO DESPACHO': return 'bg-blue-50 text-blue-700 border-blue-200'
        ...
        <div v-else class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-100">
            <thead class="bg-gray-200 border-b-2 border-gray-300">
              <tr>
                <th class="px-3 py-3 text-left text-[10px] font-black text-gray-700 uppercase tracking-tight border-r border-gray-300">DATA DE RECEBIMENTO</th>
                <th class="px-3 py-3 text-left text-[10px] font-black text-gray-700 uppercase tracking-tight border-r border-gray-300">SEI nº</th>
                <th class="px-3 py-3 text-left text-[10px] font-black text-gray-700 uppercase tracking-tight border-r border-gray-300">ORIGEM</th>
                <th class="px-3 py-3 text-left text-[10px] font-black text-gray-700 uppercase tracking-tight border-r border-gray-300">Nº PROCESSO</th>
                <th class="px-3 py-3 text-left text-[10px] font-black text-gray-700 uppercase tracking-tight border-r border-gray-300">NOME DO SERVIDOR</th>
                <th class="px-3 py-3 text-left text-[10px] font-black text-gray-700 uppercase tracking-tight border-r border-gray-300">ASSUNTO</th>
                
                <!-- Bloco Prazo 1 -->
                <th class="px-3 py-3 text-left text-[10px] font-black text-gray-700 uppercase tracking-tight border-r border-gray-300 bg-gray-300">PRAZO INTERNO</th>
                <th class="px-3 py-3 text-center text-[10px] font-black text-gray-700 uppercase tracking-tight border-r border-gray-300 bg-gray-300">STATUS</th>
                <th class="px-3 py-3 text-center text-[10px] font-black text-gray-700 uppercase tracking-tight border-r border-gray-300 bg-gray-300">AÇÃO</th>

                <!-- Bloco Prazo 2 -->
                <th class="px-3 py-3 text-left text-[10px] font-black text-gray-700 uppercase tracking-tight border-r border-gray-300 bg-gray-400/20">PRAZO FATAL</th>
                <th class="px-3 py-3 text-center text-[10px] font-black text-gray-700 uppercase tracking-tight border-r border-gray-300 bg-gray-400/20">STATUS</th>
                <th class="px-3 py-3 text-center text-[10px] font-black text-gray-700 uppercase tracking-tight border-r border-gray-300 bg-gray-400/20">AÇÃO</th>
                
                <th class="px-3 py-3 text-right text-[10px] font-black text-gray-700 uppercase tracking-tight">AÇÕES</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="processo in filteredProcessos" :key="processo.id" class="hover:bg-blue-50/30 transition-colors border-b border-gray-200">
                <td class="px-3 py-3 whitespace-nowrap text-[11px] font-bold text-gray-600 border-r border-gray-100">
                  {{ processo.data_recebimento ? new Date(processo.data_recebimento).toLocaleDateString('pt-BR') : '-' }}
                </td>
                <td class="px-3 py-3 whitespace-nowrap text-[11px] font-medium text-gray-900 border-r border-gray-100">
                  {{ processo.sei_numero || '-' }}
                </td>
                <td class="px-3 py-3 whitespace-nowrap text-[11px] font-bold text-gray-700 border-r border-gray-100">
                  {{ processo.unidade || '-' }}
                </td>
                <td class="px-3 py-3 whitespace-nowrap text-[11px] text-blue-700 font-black border-r border-gray-100">
                  {{ processo.numero_processo || '-' }}
                </td>
                <td class="px-3 py-3 text-[11px] text-gray-700 font-medium border-r border-gray-100">
                  {{ processo.interessado || '-' }}
                </td>
                <td class="px-3 py-3 text-[11px] text-gray-600 border-r border-gray-100 max-w-[150px]">
                   <div class="font-bold truncate">{{ processo.assunto || '-' }}</div>
                   <div class="text-[9px] truncate opacity-60">{{ (processo as any).complemento_assunto }}</div>
                </td>

                <!-- Colunas Prazo 1 -->
                <td class="px-3 py-3 whitespace-nowrap text-[11px] font-bold text-gray-900 border-r border-gray-100 bg-gray-50/50">
                  {{ processo.prazo_1 ? new Date(processo.prazo_1).toLocaleDateString('pt-BR') : '-' }}
                </td>
                <td class="px-3 py-3 whitespace-nowrap border-r border-gray-100 bg-gray-50/50 text-center">
                  <span v-if="processo.status_1" :class="['px-3 py-1 rounded text-[10px] font-black shadow-sm flex items-center justify-center min-w-[100px]', (useStatusCalculator()).getStatusColor(processo.status_1)]">
                    {{ processo.status_1 }}
                  </span>
                </td>
                <td class="px-3 py-3 whitespace-nowrap text-[10px] font-black text-gray-500 text-center border-r border-gray-100 bg-gray-50/50">
                  {{ processo.acao_1 || '-' }}
                </td>

                <!-- Colunas Prazo 2 -->
                <td class="px-3 py-3 whitespace-nowrap text-[11px] font-bold text-gray-900 border-r border-gray-100 bg-gray-100/30">
                  {{ processo.prazo_2 ? new Date(processo.prazo_2).toLocaleDateString('pt-BR') : '-' }}
                </td>
                <td class="px-3 py-3 whitespace-nowrap border-r border-gray-100 bg-100/30 text-center">
                  <span v-if="processo.status_2" :class="['px-3 py-1 rounded text-[10px] font-black shadow-sm flex items-center justify-center min-w-[100px]', (useStatusCalculator()).getStatusColor(processo.status_2)]">
                    {{ processo.status_2 }}
                  </span>
                </td>
                <td class="px-3 py-3 whitespace-nowrap text-[10px] font-black text-gray-500 text-center border-r border-gray-100 bg-gray-100/30">
                  {{ processo.acao_2 || '-' }}
                </td>

                <td class="px-3 py-3 whitespace-nowrap text-right">
                  <div class="flex items-center justify-end space-x-1">
                    <button @click="handleEdit(processo)" class="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-white rounded border border-transparent hover:border-gray-200 transition-all">
                      <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                      </svg>
                    </button>
                    <button @click="handleDelete(processo.id)" class="p-1.5 text-gray-400 hover:text-red-600 hover:bg-white rounded border border-transparent hover:border-gray-200 transition-all">
                      <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
