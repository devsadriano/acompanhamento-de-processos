...
export const useStatusCalculator = () => {
    ...
    const calculateStatus = (prazo: string | null, acao: AcaoType | null): StatusType | null => {
        ...
        // 1. Ações definitivas (sempre prevalecem conforme fórmula do Excel)
        if (acao === 'RESPONDIDO') return 'CONCLUÍDO'
        if (acao === 'ANÁLISE' || acao === 'BLOQUEIO') return 'AGUARDANDO DESPACHO'
        if (acao === 'DILAÇÃO') return 'AGUARDANDO NOVO PRAZO'
        if (acao === 'ELABORAR') return 'PENDENTE'
    ...
    const getStatusColor = (status: StatusType | null): string => {
        ...
        const colors: Record<StatusType, string> = {
            ...
            'AGUARDANDO DESPACHO': 'bg-blue-100 text-blue-800',
            'AGUARDANDO NOVO PRAZO': 'bg-yellow-100 text-yellow-800',
            'PENDENTE': 'bg-gray-100 text-gray-800',
            'ATRASADO': 'bg-red-600 text-white', // Vermelho vivo
            'COBRAR': 'bg-[#ff6d01] text-white',
            'ALERTA': 'bg-amber-400 text-black',
            'NO PRAZO': 'bg-[#38761d] text-white',
        }
